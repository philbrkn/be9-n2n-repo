...
def compute_point_stats(
    ...
    """
    Recompute per-point r statistics from existing replicate collision_track.h5 files.
    Returns r_mean, r_std, r_sem, all_r(list), all_detections(list)
    """
    rep_dirs = sorted([p for p in point_dir.glob("rep_*") if p.is_dir()])
    if not rep_dirs:
        raise FileNotFoundError(f"No replicate dirs found in {point_dir}")
    ...
    for rep in rep_dirs:
        ...
        all_det.append(int(detection_times.size))
        ...
        try:
            ...
            # Ensure at least max_r_k+1 entries exist for plotting convenience
            if len(r) < (max_r_k + 1):
                r = np.pad(r, (0, (max_r_k + 1) - len(r)))
            all_r.append(r)
        except Exception:
            # keep going; you can log if you want
            continue

    if not all_r:
        raise RuntimeError(f"No successful deconvolutions in {point_dir}")

    # Pad to common length (some points may produce longer r)
...
def main() -> None:
    ...
    # Compute per point
    for p in points:
        r_mean, r_std, r_sem, all_r, all_det = compute_point_stats(
            p, sr=sr, max_r_k=max(MAX_RK, 3)
        )

        r_means.append(r_mean)
        r_sems.append(r_sem)

        det = np.asarray(all_det, dtype=float)
        det_means.append(float(det.mean()))
        det_sems.append(float(det.std(ddof=0) / np.sqrt(det.size)))

        print(
            f"[{p.name}] reps={len(all_det)}  detections={det.mean():.1f}±{det_sems[-1]:.1f}"
        )
