...
from sweeps.plot_sweep_bootstrap import get_sensitivity
from utils.config import REPO_ROOT, get_be9_h5
...
def plot_ddx_perturbation_figure(
    ...
    nominal_be9_path = str(
        next((nominal_point / "xs").glob("Be9_scaled_*.h5"), Path(get_be9_h5()))
    )
...
def main() -> None:
    OUTPUT_ROOT = REPO_ROOT / "outputs" / "be_rad_ddx_sweep_11.0"
    # OUTPUT_ROOT = REPO_ROOT / "outputs" / "10e6_particles"

    FIG_DIR = REPO_ROOT / "figures"
    FIG_DIR.mkdir(parents=True, exist_ok=True)

    PATTERN = "ddx_scale_*"
    FIG_PATH = FIG_DIR / "ddx_scale_plot.png"
