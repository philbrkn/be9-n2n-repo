...
from paper_scripts import REPO_ROOT
...
if __name__ == "__main__":
    ...
    densities = [0.0003, 0.0004, 0.0005, 0.0006, 0.0007]

    results = run_sweep(
        input_dir=input_dir,
        output_root=output_root,
        base_cfg=base_cfg,
        param_name="he_density",
        values=densities,
        label_fmt="he_density_{:.4f}",
    )
    ...
    for idx, ax in enumerate(axes.flat[:3]):
        ...
        ax.set_xlabel("He-3 density (g/cm³)")
    ...
    plt.savefig(REPO_ROOT / "figures" / "he_density_sensitivity_results.png", dpi=300)
