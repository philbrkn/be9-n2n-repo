...
if __name__ == "__main__":
    ...
    for source in source_rates:
        ...
        print(
            f"[scale {source:.2f}] mean detections: {np.mean(all_det):.1f}  std: {np.std(all_det):.1f}"
        )
    ...
    for idx, ax in enumerate(axes.flat[:3]):
        ...
        ax.set_xlabel("source rate")
        ax.set_ylabel(f"r[{idx}]")
        ax.set_title(f"r[{idx}] vs source rate")
    ...
    plt.tight_layout()
    plt.savefig("figures/source_sensitivity_results.png", dpi=300)
